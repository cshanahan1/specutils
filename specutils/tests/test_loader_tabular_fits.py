...
def test_wcs_tabular_roundtrip(remote_data_path):
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', (VerifyWarning, UnitsWarning))
        optical_spec_2 = Spectrum1D.read(remote_data_path, format='wcs1d-fits')

    # write out spectrum using tabular-fits format
    # check headers
    # read back in using tabular-fits format and check data and header data